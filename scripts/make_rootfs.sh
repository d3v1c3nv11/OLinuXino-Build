...
add_platform_scripts() {
	...
	cp -av ./platform-scripts/$PLATFORM/* "$DEST/usr/local/sbin/"
...
add_udev_rules() {
	...
	cp -av ./configuration-files/$PLATFORM/udev-rules.d/* "$DEST/etc/udev/rules.d/"
...
add_modprobe_d() {
	...
	cp -av ./configuration-files/$PLATFORM/modprobe.d/* "$DEST/etc/modprobe.d/"
...
add_modules_load_d() {
	...
	cp -av ./configuration-files/$PLATFORM/modules-load.d/* "$DEST/etc/modules-load.d/"
	chown -R root.root "$DEST/etc/modules-load.d"
}
add_prebuilt_packages() {
	mkdir -p "$DEST/home/prebuilt"
	cp -av   ./prebuilt/$PLATFORM/* "$DEST/home/prebuilt/"
}
...
add_asound_state() {
	...
	cp -vf ../blobs/asound.state.$PLATFORM "$DEST/var/lib/alsa/asound.state"
}
