...
PWD=$(readlink -f .)
TEMP=$(mktemp -p $PWD -d -t "$PLATFORM-build-XXXXXXXXXX")
IMAGE="$DISTRO-$PLATFORM-bspkernel-$DATE-$COUNT.img"
...
sleep 2
(./make_rootfs.sh "$TEMP/rootfs" "$KERNELTAR" "$DISTRO" "$TEMP/boot" "$PLATFORM")
